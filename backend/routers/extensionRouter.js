...
router.post('/', async (req, res) => {
  ...
  try {
    const {
      name, publisher, identifier, version, logo, description, confirm, published, user
    } = req.body;

    // Validate required fields
    if (!name || !publisher || !identifier || !version) {
      return res.status(400).json({ error: 'Missing required fields: name, publisher, identifier, version' });
    }
    ...
    console.error('Error saving extension:', error.message, error.stack);
...
router.get('/api/validate-extension', async (req, res) => {
  ...
  try {
    ...
    let responseUrl = error.response?.request?.res?.responseUrl || '';
